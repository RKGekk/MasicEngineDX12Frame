...
AnimMenuUI::AnimMenuUI(std::shared_ptr<ProcessManager> pm) {
	...
	LPCWSTR a = L"open data/sound/349_604_2.8.0.3.mp3 type mpegvideo";
	int error = 99;
...
HRESULT AnimMenuUI::VOnRender(const GameTimerDelta& delta, std::shared_ptr<CommandList> command_list) {
	...
	if (ImGui::Begin("Player Menu")) {
		...
		if (ImGui::Button("Play")) {
			anim_player->Play();
			int error2;
			LPCWSTR b = L"play data/sound/349_604_2.8.0.3.mp3";
		...
		if (ImGui::Button("Stop")) {
			anim_player->Pause();
			int error2;
			//LPCWSTR b = L"stop data/sound/349_604_2.8.0.3.mp3";
			LPCWSTR b = L"seek data/sound/349_604_2.8.0.3.mp3 to start";
			error2 = mciSendString(b, NULL, 0, 0);
		}
